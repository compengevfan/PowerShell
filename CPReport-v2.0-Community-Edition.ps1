...
#-------------------------CHANGE THESE VALUES--------------------------------
$SMTPServer = "SMTP_SERVER_NAME_OR_IP"
...
$ScriptVersion = "v2.0 - Community Edition" # Included in the Runtime info in $HTMLFooter
$Subject = "VMware Capacity & Performance Report for " + $vCenterServerName
$FromAddress = $vCenterServerName + "@company.com"
...
ForEach ($ClusterTemp in ($Clusters)){
	...
	If ($NumberOfVMHostsInCluster -gt 1){ #Ignore Clusters with no ESXi hosts in it
		...
		# CHART
		Create-Chart -ChartType Bar -ChartTitle "Datastore Usage Percentage " -FileName ("Cluster_" + $ClusterTemp.Name + `
