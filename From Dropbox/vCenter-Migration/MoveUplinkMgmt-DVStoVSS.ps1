...
foreach($_d in $dvs) {
    ...
    foreach($_v in $vmk) {
        try {
            ...
            if($vlan) { $pg_array += New-VirtualPortGroup -VirtualSwitch $vss -Name $_v.PortGroupName -VLanId $vlan -ErrorAction Stop }
            else { $pg_array += New-VirtualPortGroup -VirtualSwitch $vss -Name $_v.PortGroupName -ErrorAction Stop }
        }
        catch { WriteHost "$localname - Failed to create port group $($_v.PortGroupName)" -color "Red" }
    }
