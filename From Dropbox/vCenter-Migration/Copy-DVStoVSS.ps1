...
foreach($_d in $dvs) {
    ...
    try {
        ...
        $dvpgs = Get-VDPortgroup -VDSwitch $_d -ErrorAction Stop | ?{$_.Name -notlike "*Uplinks*" -and $_.Name -notlike "*vmMGMT*" -and $_.Name -notlike "*vMotion*" -and $_.Name -notlike "*iSCSI0*"} | sort Name
