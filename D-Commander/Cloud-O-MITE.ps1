...
if ($InputFile -eq "" -or $null -eq $InputFile) { Clear-Host; Write-Host "Please select a JSON file..."; $InputFile = Get-DfFileName -Filter "json" }
...
Invoke-DfLogging @LoggingInfoSplat -LogString "Importing JSON Data File: $InputFile..."
$DataFromFile = ConvertFrom-JSON (Get-Content $githome\vmbuildfiles\$InputFile -raw)
if ($null -eq $DataFromFile) { Invoke-DfLogging @LoggingErrSplat -LogString "Error importing JSON file. Please verify proper syntax and file name."; exit }
...
if ($($DataFromFile.GuestInfo.Domain) -ne "none") {
    while ($true) {
        if ($null -eq $DomainCredentials) {
            Invoke-DfLogging @LoggingWarnSplat -LogString "Obtaining Domain Credentials. Note: Username MUST be in 'user principle name' format. For example: me@domain.com"
        ...
        Invoke-DfLogging @LoggingInfoSplat -LogString "Testing domain credentials..."
        ...
        if ($null -eq $domain.name) { Invoke-DfLogging @LoggingWarnSplat -LogString "Domain Credentials Failed. Please try again or press Control-C to exit..."; Start-Sleep -Seconds 2 }
        else { Invoke-DfLogging @LoggingSuccSplat -LogString "Credential test was successful..."; break }
    }
}
...
Invoke-DfLogging @LoggingInfoSplat -LogString "Locating Template $TemplateToFind"
$TemplateToUse = Get-Template $TemplateToFind -ErrorAction Ignore
if ($null -ne $TemplateToUse) { Invoke-DfLogging @LoggingSuccSplat -LogString "Template found..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "Template NOT found!!! Script Exiting!!!"; return 66 }

#Find the Folder. If it is not found, exit.
Invoke-DfLogging @LoggingInfoSplat -LogString "Parsing folder path to find proper location..."
$Folder = $null
$Folder = Get-FolderByPath -Path $($DataFromFile.VMInfo.FolderPath) -ErrorAction Ignore
if ($null -ne $Folder) { Invoke-DfLogging @LoggingSuccSplat -LogString "Location found..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "Location NOT found!!! Script Exiting!!!"; return 66 }

#Find the customization spec. If it is not found, exit.
Invoke-DfLogging @LoggingInfoSplat -LogString "Locating Customization Spec $($DataFromFile.GuestInfo.CustomizationSpec)..."
$OSCustSpec = Get-OSCustomizationSpec $($DataFromFile.GuestInfo.CustomizationSpec) -ErrorAction Ignore
if ($null -ne $OSCustSpec) { Invoke-DfLogging @LoggingSuccSplat -LogString "Customization Spec found..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "Customization Spec NOT found!!! Script Exiting!!!"; return 66 }

#Find the Portgroup. Check for vDS first; if not found, check for standard. If it is not found, exit.
Invoke-DfLogging @LoggingInfoSplat -LogString "Locating PortGroup $($DataFromFile.VMInfo.PortGroup)..."
...
if ($null -ne $PortGroup) { Invoke-DfLogging @LoggingSuccSplat -LogString "PortGroup found..."; $PortType = "vds" }
if ($null -eq $PortGroup) {
    ...
    if ($null -ne $PortGroup) { Invoke-DfLogging @LoggingSuccSplat -LogString "PortGroup found..."; $PortType = "std" }
    else { Invoke-DfLogging @LoggingErrSplat -LogString "PortGroup NOT found!!! Script Exiting!!!"; return 66 }
}

#Find the Datastore. First, look for a DS Cluster. If not found, look for DS. If it is not found, exit.
Invoke-DfLogging @LoggingInfoSplat -LogString "Locating Datastore Cluster $($DataFromFile.VMInfo.Datastore)..."
...
if ($null -ne $DataStore) { Invoke-DfLogging @LoggingSuccSplat -LogString "Storage found..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "Storage NOT found!!! Script Exiting!!!"; return 66 }
...
if ($null -eq $CheckName) { Invoke-DfLogging @LoggingSuccSplat -LogString "New VM name not found..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "New VM name found!!! Script Exiting!!!"; return 66 }
...
if ($null -eq $CheckIPAddress) { Invoke-DfLogging @LoggingSuccSplat -LogString "New VM IP not found..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "New VM IP found!!! Script Exiting!!!"; return 66 }
...
if ($null -eq $CheckIPAddress) { Invoke-DfLogging @LoggingSuccSplat -LogString "New VM IP does not respond to ping..." }
else { Invoke-DfLogging @LoggingErrSplat -LogString "New VM IP does respond to ping!!! Script Exiting!!!"; return 66 }
...
Invoke-DfLogging @LoggingInfoSplat -LogString "Creating temporary customization spec..."
...
if ($($DataFromFile.GuestInfo.Domain) -ne "none") {
    Invoke-DfLogging @LoggingInfoSplat -LogString "Configuring domain settings in temporary customization spec..."
...
Invoke-DfLogging @LoggingInfoSplat -LogString "Configuring network settings in temporary customization spec..."
...
#Create the VM
Invoke-DfLogging @LoggingInfoSplat -LogString "Deploying VM..."
...
if ($null -eq (Get-VM $VmName -ErrorAction SilentlyContinue)) { Invoke-DfLogging @LoggingErrSplat -LogString "VM Deploy failed!!! Script exiting!!!"; Invoke-DfLogging @LoggingErrSplat -LogString $NewVMFail; return 66 }
else {
    Invoke-DfLogging @LoggingInfoSplat -LogString "Updating VM Specs..."
    ...
    Invoke-DfLogging @LoggingInfoSplat -LogString "Checking for an optical drive..."
    if (!(Get-CDDrive -VM $VmName)) { Invoke-DfLogging @LoggingInfoSplat -LogString "Adding optical drive..."; New-CDDrive -VM $VmName -Confirm:$false | Out-Null }
    else { Invoke-DfLogging @LoggingInfoSplat -LogString "VM already has an optical drive..." }
    Start-Sleep 5

    Invoke-DfLogging @LoggingInfoSplat -LogString "Powering on VM..."
    Start-VM $VmName | Out-Null
    Invoke-DfLogging @LoggingInfoSplat -LogString "Waiting for customization to start..."
    while ($True) {
    ...
    Invoke-DfLogging @LoggingInfoSplat -LogString "Customization has started. Waiting for it to complete..."
    while ($True) {
        $vmEvents = Get-VIEvent -Entity $VmName
        ...
        if ($FailureEvent) {
            Invoke-DfLogging @LoggingErrSplat -LogString -Message "Customization of VM failed!!! Script exiting!!!"; exit
        }
    ...
    Invoke-DfLogging @LoggingSuccSplat -LogString "Customization of VM Completed Successfully..."
}

Invoke-DfLogging @LoggingInfoSplat -LogString "Removing temporary customization spec..."
Remove-OSCustomizationSpec -OSCustomizationSpec $VmName -Confirm:$false

Invoke-DfLogging @LoggingInfoSplat -LogString "Waiting a while after customization for Windows to stabilize..."
Start-Sleep 60

if ($($DataFromFile.GuestInfo.Domain) -ne "none") {
    #Move server to appropriate OU, if OU in the JSON does not exist, the server gets moved to the "Servers" OU.
    Invoke-DfLogging @LoggingInfoSplat -LogString "Verifying OU '$($DataFromFile.GuestInfo.OU)' exists..."
    ...
    try {
        ...
        Invoke-DfLogging @LoggingInfoSplat -LogString "Moving server to OU '$($DataFromFile.GuestInfo.OU)'..."
        ...
        Invoke-DfLogging @LoggingWarnSplat -LogString "OU '$($DataFromFile.GuestInfo.OU)' does not exist!!! Moving server to 'servers' OU..."
        $DN = ConvertToDN -OUPath "Servers" -Domain $($DataFromFile.GuestInfo.Domain)
        Get-ADComputer -Identity $VmName -Server $($DataFromFile.GuestInfo.DNS1) -Credential $DomainCredentials | Move-ADObject -TargetPath $DN -Server $($DataFromFile.GuestInfo.DNS1) -Credential $DomainCredentials
    }
}

Invoke-DfLogging @LoggingSuccSplat -LogString "Your VM has been successfully deployed!!!"
