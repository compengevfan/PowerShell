...
if ($VMNode1 -eq "" -or $null -eq $VMNode1) { Clear-Host; Write-Host "Please select a JSON file for node 1..."; $VMNode1 = Get-DfFileName -Filter "json" }
if ($VMNode2 -eq "" -or $null -eq $VMNode2) { Clear-Host; Write-Host "Please select a JSON file for node 2..."; $VMNode2 = Get-DfFileName -Filter "json" }
if ($ClusterConfig -eq "" -or $null -eq $ClusterConfig) { Clear-Host; Write-Host "Please select a JSON file for the cluster config..."; $ClusterConfig = Get-DfFileName -Filter "json" }
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Importing JSON Data File: $VMNode1..."
$DataFromFile = ConvertFrom-JSON (Get-Content $githome\vmbuildfiles\$VMNode1 -raw)
if ($null -eq $DataFromFile) { Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "Error importing JSON file. Please verify proper syntax and file name."; exit }

#Check to make sure we have a JSON file location and if so, get the info.
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Importing JSON Data File: $VMNode2..."
$DataFromFile2 = ConvertFrom-JSON (Get-Content $githome\vmbuildfiles\$VMNode2 -raw)
if ($null -eq $DataFromFile2) { Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "Error importing JSON file. Please verify proper syntax and file name."; exit }

#Check to make sure we have a JSON file location and if so, get the info.
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Importing JSON Data File: $ClusterConfig..."
$DataFromFile3 = ConvertFrom-JSON (Get-Content $githome\vmbuildfiles\$ClusterConfig -raw)
if ($null -eq $DataFromFile3) { Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "Error importing JSON file. Please verify proper syntax and file name."; exit }

#Get the array of disks from the json file and verify that it's not too many...
$DisksToAdd = $DataFromFile3.AdditionalDisks
if ($DisksToAdd.Count -gt 12) { Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "This script doesn't support more than 15 data disks, including the G, M and Q Drives. Please build VM with Cloud-o-MITE and add disks manually or reduce the number of data disks."; exit }
...
if ($null -eq $DomainCredentials)
{
    while($true)
    {
        Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Warn -LogString "Obtaining Domain Credentials for the $($DataFromFile.GuestInfo.Domain) domain. Note: Username MUST be in 'user principle name' format. For example: me@domain.com"
        ...
        if ($null -eq $domain.name) { Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Warn -LogString "Domain Credentials Failed. Please try again or press Control-C to exit..."; Start-Sleep -Seconds 2 }
...
# Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Removing G Drive from Node 1..."
# Get-HardDisk -VM $VmName | Where-Object { $_.Name -eq "Hard Disk 3" } | Remove-HardDisk -DeletePermanently -Confirm:$false | Out-Null
# Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Removing G Drive from Node 2..."
# Get-HardDisk -VM $VmName2 | Where-Object { $_.Name -eq "Hard Disk 3" } | Remove-HardDisk -DeletePermanently -Confirm:$false | Out-Null

#Shutdown the nodes 
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Shutting down Node 1..."
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Shutting down Node 2..."
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Creating a new G Drive on Node 1 and moving to a new SCSI controller with Physical Bus Sharing..."
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Changing the SCSI ID of the G drive from 1 to 0..."
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Adding G drive from Node 1 to Node 2 and moving to a new SCSI controller with Physical Bus Sharing..."
...
#Change the SCSI ID of the moved disk
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Changing the SCSI ID of the G drive from 2 to 0..."
$SecondNode = Get-VM $VmName2
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Creating additional shared disks..."
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Creating Q drive on Node 1..."
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Adding Q drive to Node 2..."
New-HardDisk -VM $SecondNode -Controller $NewController2 -DiskPath $($NewDisk.Filename) | Out-Null

Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Creating M drive on Node 1..."
$NewDisk = New-HardDisk -VM $FirstNode -Controller $NewController1 -CapacityGB 5 -StorageFormat EagerZeroedThick -Datastore $($DataFromFile.VMInfo.Datastore)
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Adding M drive to Node 2..."
New-HardDisk -VM $SecondNode -Controller $NewController2 -DiskPath $($NewDisk.Filename) | Out-Null

foreach($DiskToAdd in $DisksToAdd)
{
    Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Creating $($DiskToAdd.MPName) disk on Node 1..."
    ...
    Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Adding $($DiskToAdd.MPName) disk to Node 2..."
    New-HardDisk -VM $SecondNode -Controller $NewController2 -DiskPath $($NewDisk.Filename) | Out-Null
}

#Power the VMs on
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Shared disks created. Powering on Node 1..."
...
Invoke-DfLogging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Powering on Node 2..."
