...
#Check to make sure we have a JSON file location and if so, get the info.
DoLogging -LogType Info -LogString "Importing JSON Data File: $FSFile..."
...
if ($DisksToAdd.Count -gt 23) { DoLogging -LogType Err -LogString "This script doesn't support more than 23 data disks, including the G Drive. Please build VM with Cloud-o-MITE and add disks manually or reduce the number of data disks."; exit }
...
{
    if ($DiskNumber -eq 2 -and $($DiskToAdd.Size) -le 10) { DoLogging -LogType Info -LogString "G Drive will not be resized..." }
    elseif ($DiskNumber -eq 2)
    {
        DoLogging -LogType Info -LogString "Expanding G drive to $($DiskToAdd.Size) GB..."
    ...
    { DoLogging -LogType Warn -LogString "Drive letters C, D and P are already in use. Skipping..." }
    elseif ($DiskNumber -ne 2)
    {
        DoLogging -LogType Info -LogString "Adding drive $($DiskToAdd.DriveLetter)..."
