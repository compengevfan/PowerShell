...
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Importing JSON Data File: $VMFile..."
...
#Check to make sure we have a JSON file location and if so, get the info.
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Importing JSON Data File: $FSFile..."
$DataFromFile2 = ConvertFrom-JSON (Get-Content $FSFile -raw)
if ($DataFromFile2 -eq $null) { Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "Error importing JSON file. Please verify proper syntax and file name."; exit }

#Get the array of disks from the json file and verify that it's not too many...
$DisksToAdd = $DataFromFile2.DiskLayout
if ($DisksToAdd.Count -gt 23) { Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "This script doesn't support more than 23 data disks, including the G Drive. Please build VM with Cloud-o-MITE and add disks manually or reduce the number of data disks."; exit }
...
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Calling Cloud-O-MITE to build the VM..."
$Code = .\Cloud-O-MITE.ps1 -InputFile $VMFile -DomainCredentials $DomainCredentials
if ($Code -eq 66) { Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Err -LogString "VM build failed. Exiting build script."; exit }
...
{
    if ($DiskNumber -eq 2 -and $($DiskToAdd.Size) -le 10) { Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "G Drive will not be resized..." }
    elseif ($DiskNumber -eq 2)
    {
        Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Expanding G drive to $($DiskToAdd.Size) GB..."
    ...
    { Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Warn -LogString "Drive letters C, D and P are already in use. Skipping..." }
    elseif ($DiskNumber -ne 2)
    {
        Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Adding drive $($DiskToAdd.DriveLetter)..."
...
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString $InvokeOutput
...
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Triggering server restart to complete the feature install..."
Restart-VMGuest -VM $($DataFromFile.VMInfo.VMName) -Confirm:$false

Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Succ -LogString "Your file server has been successfully configured!!!"
