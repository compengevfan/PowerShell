...
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Running check as $($Credential_To_Use.Username)"
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Getting the currently running job list..."
...
Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "There are $($ActiveJobs.Count) active jobs."

if ($($ActiveJobs.Count) -gt 0)
{
    Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Getting the current time for comparison..."
    $CurrentTime = Get-Date
}

foreach ($ActiveJob in $ActiveJobs)
{
    Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Checking to see if job number $($ActiveJob.jobNumber) has been running for more than $MaxTime minutes (unless project name contains 'Archive' or 'Zip')..."
    ...
    {
        Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Job number $($ActiveJob.jobNumber) has been running more than $MaxTime minutes..."
        ...
        if ($CurrentJobLog[($CurrentJobLog.Count - 1)].EndsWith("'createFileList 1.0'"))
        {
            ...
            Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Job number $($ActiveJob.jobNumber) appears to be stuck. Checking to see if it was cancelled on the previous run..."
            if ($Cancelled -match $($ActiveJob.jobNumber))
            {
                Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Warn -LogString "Job number $($ActiveJob.jobNumber) was cancelled on the previous run!!! Sending email for human intervention and exiting script!!!"
                $emailBody = @"
...
"@
            ...
            {
                Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Issuing command to cancel job $($ActiveJob.jobNumber)..."
                try
                {
                ...
                Invoke-Logging -ScriptStarted $ScriptStarted -ScriptName $ScriptName -LogType Info -LogString "Logging job number to ensure cancel was successful on next run..."
