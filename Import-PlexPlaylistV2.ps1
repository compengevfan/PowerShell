...
foreach ($Entry in $Playlist) {
    ...
    if ($Entry.TrackNo -ne "0") { $matchingKeys = $MusicLibrary.MediaContainer.Metadata | Where-Object { $_.grandparentTitle -eq $Entry.artist -and $_.title -eq $Entry.song -and $_.parentTitle -eq $Entry.album -and $_.index -eq $Entry.TrackNo } | Select-Object -ExpandProperty ratingKey }
